...
- 배포: GitHub Pages
- 현재 구현: 튜토리얼 6판 + 물길마을 도착 흐름 + 길목 G1~G7 + 장인골 W1~W7 + 장터 M1~M8 + 여관 방 MVP
- v1.0 목표: **물길마을의 첫 여행** — 세 갈래의 완결, 돌아올 여관, 첫 자유 의뢰
- 바로 다음 단계: **세 core 합류와 1장 결말 구현**. 세 구역 완료 뒤 여관 장소 문구가 바뀌고, 시간 점프 없이 여관에서 첫 여행을 정리한 뒤 여관 주인의 첫 직접 의뢰로 이어진다.
...
4. 길목 G1~G7에서 길·거리·경로·장애물·수레·인도 흐름을 완결하고 `gate-core`를 기록한다.
...
8. 다음으로 세 core가 모두 모인 뒤 **여관 장소 문구 변화 → 여관 합류 장면 → 방에서 1장 결말 → 여관 주인의 첫 직접 의뢰**를 구현한다.
