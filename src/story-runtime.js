(() => {
  ...
  const speakers = { narrator: ['旁白', '이야기'], boy: ['少年', '소년'], merchant: ['行商', '행상인'], unknown: ['遠處的聲音', '멀리서 들리는 목소리'] };
  const backgrounds = { origin: 'region-origin', forest: 'region-origin', roadside: 'region-gate-town' };
  let session = null;
  function render() {
    if (!session) return;
