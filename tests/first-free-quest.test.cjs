...
test('C08 starts only after the chapter ending and keeps offer, forest, and report out of generic journey', () => {
  ...
  assert.deepEqual(snapshot.map(item => [item.id, item.regionId, item.hidden]), [
  ...
  assert.deepEqual(snapshot[0].sequence[0].requires, ['story:chapter1-room-finale']);
  assert.deepEqual(snapshot[1].sequence[0].requires, ['story:first-free-quest-accepted']);
  assert.deepEqual(snapshot[2].sequence[0].requires, ['stage:first-free-quest-forest']);
...
test('the northern forest quest deliberately reuses chapter-one vocabulary instead of adding a new word family', () => {
  ...
  assert.deepEqual(snapshot.words, ['範圍', '數量', '不足', '足夠', '獲得', '退出']);
  snapshot.words.forEach(word => assert.ok(snapshot.allWords.includes(word), `missing reused WORDS entry: ${word}`));
...
test('world runtime reveals the existing northern forest only after acceptance and installs the board only after reporting', () => {
  ...
  assert.match(runtime, /first-free-quest-completed/);
  assert.match(runtime, /quest-board-unlocked/);
